...
# ─────────────────────────────────────────────
# 재계 주요 기업 (ESG·상생 기사 필터용 — 대기업 언급 시에만 섹션1)
# ─────────────────────────────────────────────
MAJOR_COMPANIES = COMPANY_KEYWORDS + COMPETITOR_KEYWORDS + [
    # 삼성
    "삼성전자", "삼성물산", "삼성SDS", "삼성생명", "삼성화재",
    "삼성SDI", "삼성바이오로직스", "제일기획", "삼성웰스토리",
    # SK
    "SK하이닉스", "SK텔레콤", "SKT", "SK이노베이션", "SK에너지",
    "SK네트웍스", "SK바이오팜", "SK스퀘어",
    # 현대차
    "현대자동차", "현대차", "기아", "현대모비스", "현대건설",
    "현대글로비스", "현대오토에버", "HD현대",
    # LG
    "LG전자", "LG화학", "LG디스플레이", "LG유플러스", "LGU+",
    "LG생활건강", "LG CNS", "LG이노텍",
    # 롯데
    "롯데그룹", "롯데케미칼", "롯데칠성", "롯데웰푸드", "롯데푸드",
    "롯데건설", "롯데호텔", "롯데지알에스",
    # 한화
    "한화그룹", "한화에어로스페이스", "한화솔루션", "한화생명",
    "한화갤러리아", "한화큐셀",
    # GS
    "GS그룹", "GS칼텍스", "GS건설", "GS에너지",
    # CJ
    "CJ그룹", "CJ제일제당", "CJ대한통운", "CJ ENM", "CJ CGV",
    "CJ프레시웨이", "CJ푸드빌",
    # 신세계
    "신세계그룹", "신세계인터내셔날", "조선호텔", "스타벅스코리아",
    # 포스코
    "포스코홀딩스", "POSCO", "포스코", "포스코인터내셔널",
    # 현대중공업
    "현대중공업", "현대미포조선", "현대삼호중공업",
    # 아모레퍼시픽
    "아모레퍼시픽", "이니스프리", "에뛰드", "설화수",
    # 이랜드
    "이랜드그룹", "이랜드리테일", "NC백화점", "뉴코아",
    # SPC
    "SPC그룹", "파리바게뜨", "배스킨라빈스", "파리크라상",
    # 식품/유통
    "농심", "오리온", "빙그레", "롯데웰푸드", "해태",
    "풀무원", "동원그룹", "대상그룹", "CJ제일제당",
    "오뚜기", "삼양식품", "하이트진로", "오비맥주",
    # 패션
    "삼성물산 패션", "LF", "코오롱FnC", "한섬",
    "무신사", "W컨셉", "에프앤에프",
    # 플랫폼/이커머스
    "네이버", "카카오", "배달의민족", "우아한형제들",
    "토스", "당근마켓", "컬리",
    # 금융
    "KB금융", "신한금융", "하나금융", "우리금융",
    "미래에셋", "한국투자증권", "교보생명",
    # 기타 대기업
    "두산그룹", "두산에너빌리티", "효성그룹", "LS그룹",
    "한진그룹", "대한항공", "한진칼",
    "코오롱그룹", "태광그룹", "세아그룹",
]
...
# 섹션1 트렌드/정책 키워드
# (카테고리, 키워드, base_score, require_company)
# require_company=True 이면 MAJOR_COMPANIES 중 하나가 기사에 있어야 섹션1 분류
SECTION1_RULES = [
    ("정부정책_규제", [
    ...
    ("ESG_상생", [
        ...
        "동반성장", "상생 협력", "지속가능경영",
        "RE100", "친환경 매장",
    ], 5, True),   # 반드시 재계 기업명 포함 시에만 섹션1
    ("조직문화_노동", [
        "감정노동 보호", "워라밸", "MZ세대 직원",
        "직원 복지", "육아휴직 확대", "재택근무 도입",
        "유연근무제", "번아웃", "이직률 감소",
    ], 4, True),   # 반드시 재계 기업명 포함 시에만 섹션1
]
...
def classify_article(title: str, description: str, hint_category: str = "") -> dict:
    ...
    # ── 섹션1: 유통 트렌드/정책 ──
    for category, keywords, base_score, require_company in SECTION1_RULES:
        h = _hits(text, keywords)
        if h == 0:
            continue
        # require_company=True: 재계 기업명이 없으면 기타 처리
        if require_company and not any(kw in text for kw in MAJOR_COMPANIES):
            continue
